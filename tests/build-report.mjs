...
const headers = ['xray_local_land','xml_global_land_id','cad_name','component','package','left_mm','top_mm','center_x_mm','center_y_mm','width_mm','length_mm','measurement_pixel','confidence','duplicate_cad_name_count','source_row'];
const esc = (v) => { const s=v==null?'':String(v); return /[",\r\n]/.test(s)?`"${s.replace(/"/g,'""')}"`:s; };
const lines = [headers.join(',')];
for (const m of rows) lines.push([m.localIndex,m.globalId,m.cadName,m.componentName,m.packageName,m.left,m.top,m.centerX,m.centerY,m.width,m.length,m.measurement,m.confidence,m.duplicateCadNameCount,m.sourceRow].map(esc).join(','));
